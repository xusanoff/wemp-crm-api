...
Qoida:
  - "keldi"    → DARHOL xabar yuboriladi
  - "kelmadi"  → DARHOL xabar YUBORILMAYDI (soat 20:00 da scheduler yuboradi)
"""
...
def notify_attendance(saved_records: list, lesson_info: dict):
    ...
    saved_records ichida:
      student_id, student_name, status, arrival_time,
      module_title, module_lesson_title, lesson_type
    """
    try:
        for rec in saved_records:
            # Faqat "keldi" statusdagilarni darhol yuborish
            if rec.get("status") != "keldi":
                continue
