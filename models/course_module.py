...
Fayl base64 formatida PostgreSQL DB'da saqlanadi — Vercel-compatible.
"""
import pytz
...
class ModuleLesson(db.Model):
    ...
    Fayl base64 formatida DB'da saqlanadi (Vercel serverless bilan mos).
    """
    ...
    description = db.Column(db.Text, nullable=True)
    # Fayl — base64 formatida DB'da saqlanadi (Vercel-compatible)
    file_data   = db.Column(db.Text, nullable=True)           # base64 encoded
    ...
    def to_dict(l, include_file_data=False):
        _ = {
            ...
            "has_file":    l.file_data is not None,
            "created_at":  str(l.created_at),
        }
        if include_file_data and l.file_data:
            _["file_data"] = l.file_data  # base64 string — frontend render uchun
        return _
